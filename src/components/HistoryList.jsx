  ...
  return (
    <section className="mt-4">
      ...
      <div className="vstack gap-2">
        {items.map(item => {
          const src = getLanguage(item.sourceCode)
          const tgt = getLanguage(item.targetCode)
          return (
            <div key={item.id} className="card history-card border-0 shadow-sm">
              <div className="card-body py-3">
                <div className="row g-2">
                  <div className="col-md-6">
                    <div className="small text-secondary mb-1">{src.flag} {src.name}</div>
                    <div className="history-text">{item.source || <em className="text-secondary">—</em>}</div>
                  </div>
                  <div className="col-md-6 border-start-md ps-md-3">
                    <div className="small text-secondary mb-1">{tgt.flag} {tgt.name}</div>
