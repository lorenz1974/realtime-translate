...
const PRESET_API_KEY = import.meta.env.VITE_OPENAI_API_KEY || ''
...
export default function App() {
  const [apiKey, setApiKey] = useState(
    () => localStorage.getItem('rt_api_key') || PRESET_API_KEY,
  )
  const [sourceLang, setSourceLang] = useState(() =>
    validCode(localStorage.getItem('rt_source'), 'it'),
  )
  const [targetLang, setTargetLang] = useState(() =>
    validCode(localStorage.getItem('rt_target'), 'en'),
  )
  const [deviceId, setDeviceId] = useState(
    () => localStorage.getItem('rt_device') || '',
  )
  const [devices, setDevices] = useState([])
  const [showSettings, setShowSettings] = useState(false)

  const [autoPlayAudio, setAutoPlayAudio] = useState(
    () => localStorage.getItem('rt_autoplay') !== '0',
  )
  const [transcribeInput, setTranscribeInput] = useState(
    () => localStorage.getItem('rt_transcribe') !== '0',
  )
  const [showHistory, setShowHistory] = useState(
    () => localStorage.getItem('rt_history') !== '0',
  )
  ...
  useEffect(() => {
    localStorage.setItem('rt_source', sourceLang)
  }, [sourceLang])
  useEffect(() => {
    localStorage.setItem('rt_target', targetLang)
  }, [targetLang])
  useEffect(() => {
    localStorage.setItem('rt_device', deviceId)
  }, [deviceId])
  useEffect(() => {
    localStorage.setItem('rt_autoplay', autoPlayAudio ? '1' : '0')
  }, [autoPlayAudio])
  useEffect(() => {
    localStorage.setItem('rt_transcribe', transcribeInput ? '1' : '0')
  }, [transcribeInput])
  useEffect(() => {
    localStorage.setItem('rt_history', showHistory ? '1' : '0')
  }, [showHistory])
  ...
  return (
    <div className='app-shell'>
      ...
      <main className='container py-3 py-md-4'>
        <section className='hero text-center mb-4'>
          ...
          <p className='text-secondary mb-0'>
            ...
            Per conversare in due, inverti le lingue con il pulsante{' '}
            <i className='bi bi-arrow-left-right'></i> al cambio turno.
            <br className='d-none d-md-inline' />
        ...
        <div className='card glass-card border-0 shadow-lg mb-4'>
          <div className='card-body p-3 p-md-4'>
            ...
            <div className='d-flex justify-content-center align-items-center gap-2 flex-wrap'>
              ...
              {isConnected && (
                <span
                  className={`badge rounded-pill ${isMuted ? 'bg-warning text-dark' : 'bg-success-subtle text-success-emphasis'}`}
                >
                  <i
                    className={`bi ${isMuted ? 'bi-mic-mute-fill' : 'bi-mic-fill'} me-1`}
                  ></i>
            ...
            {error && (
              <div
                className='alert alert-danger d-flex align-items-start mt-3 mb-0'
                role='alert'
              >
            ...
            {!apiKey && (
              <div
                className='alert alert-info d-flex align-items-center mt-3 mb-0'
                role='alert'
              >
        ...
        <footer className='app-footer text-center mt-5 mb-3 small'>
          ...
          <div className='text-secondary mt-1'>
            ...
            <span className='mx-2'>·</span>
            <span>
              <i className='bi bi-stars me-1'></i>OpenAI Realtime · React ·
              Bootstrap
            </span>
          </div>
        </footer>
      ...
      <SettingsPanel
        ...
        setTranscribeInput={setTranscribeInput}
